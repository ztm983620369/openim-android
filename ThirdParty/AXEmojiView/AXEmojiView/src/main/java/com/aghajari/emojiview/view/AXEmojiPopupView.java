...
public class AXEmojiPopupView extends FrameLayout implements AXPopupInterface {
    ...
    void updateKeyboardStateOpened(final int keyboardHeight) {
        final boolean measureForPopup = needToOpen;
        isKeyboardOpen = true;
        if (this.keyboardHeight <= 0 || needToOpen || !isShowing) {
        ...
        } else if (!isShowing || isShowingSearchView()) {
    ...
    int prepareForKeyboard() {
        hideSearchView(false);
        int reservedHeight = 0;
        if (isShowing || getParent() != null) {
            reservedHeight = Math.max(lp.height, findHeight(popupWindowHeight));
        }
    ...
    boolean pendingKeyboardSwitch = false;

    private int animFrom;

    private boolean hasKnownPopupHeight() {
        return popupWindowHeight > 0 || minHeight > 0;
    }
    ...
    void openKeyboard() {
        ...
        editText.requestFocus();
        showKeyboardNow();
        editText.post(this::showKeyboardNow);
        editText.postDelayed(this::showKeyboardNow, 120);
    }
    ...
    private void showKeyboardNow() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && editText.getWindowInsetsController() != null) {
    ...
    boolean shouldReserveKeyboardHeight() {
        return !isShowing || isShowingSearchView();
    }
